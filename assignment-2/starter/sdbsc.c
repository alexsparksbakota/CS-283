...
{
    int mode = _S_IREAD | _S_IWRITE;  // Windows read/write permissions
    int flags = O_RDWR | O_CREAT;     // Open for read/write, create if needed

    if (should_truncate)
        flags |= O_TRUNC;  // Truncate file if requested

    int fd = _open(dbFile, flags, mode);
    return (fd == -1) ? ERR_DB_FILE : fd;
}
...
{
    if (validate_range(id, 0) != DB_NO_ERROR) {
        return -1;
    }
    ...
    if (read(fd, s, STUDENT_RECORD_SIZE) != STUDENT_RECORD_SIZE || s->id == DELETED_STUDENT_ID) {
        return -1;
    }
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    student_t student;
    if (read(fd, &student, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE && student.id != DELETED_STUDENT_ID) {
...
int print_db(int fd)
{
    ...
    printf("ID FIRST_NAME LAST_NAME GPA\n");
    while (read(fd, &student, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE) {
        if (student.id != DELETED_STUDENT_ID) {
...
{
    printf("%d %s %s %.2f\n", s->id, s->fname, s->lname, s->gpa / 100.0);
}
...
{
    int tmp_fd = open(TMP_DB_FILE, O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if (tmp_fd < 0) {
        perror("open");
        return -1;
    }
